...
function normalizePageKind(value: unknown): string {
  const safe = String(value || "").trim().toLowerCase();
  if (safe === "cause") return "condition";
...
function normalizeSections(input: unknown): Array<{ id: string; heading: string; headingType: "h2" | "h3"; body: string }> {
...
async function run() {
  ...
  for await (const doc of cursor) {
    ...
    const h1Heading = truncate(
      String(doc.h1Heading || doc.headingStructure?.h1 || sections[0]?.heading || doc.titleTag || fallbackTitle),
      120,
    );
    ...
    const descriptionSource =
      doc.metaDescription || doc.overview || doc.quickAnswer || sections.map((section: any) => section.body).join(" ");
    const metaDescription = truncate(stripHtml(String(descriptionSource || "")), 160);

    const altText = Array.isArray(doc.altText)
      ? doc.altText.map((item: unknown) => String(item || "").trim()).filter(Boolean)
      : Array.isArray(doc.imageAltText)
        ? doc.imageAltText.map((item: unknown) => String(item || "").trim()).filter(Boolean)
        : [];
